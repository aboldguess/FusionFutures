...
export function EventsGallery() {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      {events.length === 0 ? (
        <p className="md:col-span-2 rounded-3xl border border-dashed border-white/10 bg-slate-900/40 p-6 text-sm text-slate-300">
          Events you add will appear here instantly. Use the form above to draft a new activity and then share it with the
          community.
        </p>
      ) : (
        events.map((event) => {
